...
void App1Main::CreateRenderers(const std::shared_ptr<DX::DeviceResources>& deviceResources)
{
	...
	m_screenManager.reset(new ScreenManager(deviceResources));
	m_screenManager.get()->Initialize();

	m_screenManager.get()->AddScreen(ref new App1::BackgroundScreen(m_screenManager));
...
void App1Main::Update()
{
	...
	m_timer.Tick([&]()
	{
		...
		m_screenManager.get()->Update(m_timer);
	});
}
...
bool App1Main::Render()
{
	...
	return m_screenManager.get()->Draw(m_timer);
